<?php
	...
	*  option) any later version.                                              *
	...
	{
		$GLOBALS['prisma_tpl'] = createobject('phpgwapi.Template',PHPGW_TEMPLATE_DIR);

		$GLOBALS['prisma_tpl']->set_file(
			array(
				'navbar' => 'navbar.tpl'
			)
		);

		$GLOBALS['prisma_tpl']->set_block('navbar','navbar_header','navbar_header');
		$GLOBALS['prisma_tpl']->set_block('navbar','extra_blocks_header','extra_block_header');
		$GLOBALS['prisma_tpl']->set_block('navbar','extra_block_row','extra_block_row');
		$GLOBALS['prisma_tpl']->set_block('navbar','extra_block_row_raw','extra_block_row_raw');
		$GLOBALS['prisma_tpl']->set_block('navbar','extra_block_row_no_link','extra_block_row_no_link');
		$GLOBALS['prisma_tpl']->set_block('navbar','extra_block_spacer','extra_block_spacer');
		$GLOBALS['prisma_tpl']->set_block('navbar','extra_blocks_footer','extra_blocks_footer');
		$GLOBALS['prisma_tpl']->set_block('navbar','sidebox_hide_header','sidebox_hide_header');
		$GLOBALS['prisma_tpl']->set_block('navbar','sidebox_hide_footer','sidebox_hide_footer');
		$GLOBALS['prisma_tpl']->set_block('navbar','appbox','appbox');
		$GLOBALS['prisma_tpl']->set_block('navbar','navbar_footer','navbar_footer');

		$var['img_root'] = $GLOBALS['phpgw_info']['server']['webserver_url'] . '/phpgwapi/templates/prisma/images';
...
// table width=100% fixed layout bug (ndee130204)
			...
			$GLOBALS['prisma_tpl']->set_var($var);
			$GLOBALS['prisma_tpl']->pfp('out','navbar_header');
			...
			* The sidebox menu's                                   *
			...
			{
				$GLOBALS['prisma_tpl']->set_var('show_menu_event',$show_menu_event);
				$GLOBALS['prisma_tpl']->set_var('lang_show_menu',lang('show menu'));
				$GLOBALS['prisma_tpl']->pparse('out','sidebox_hide_header');

				display_sidebox('',$menu_title,$file);
				$GLOBALS['phpgw']->hooks->single('sidebox_menu',$GLOBALS['phpgw_info']['flags']['currentapp']);

				$GLOBALS['prisma_tpl']->pparse('out','sidebox_hide_footer');

				$var['sideboxcolstart']='';

				$GLOBALS['prisma_tpl']->set_var($var);
				$GLOBALS['prisma_tpl']->pparse('out','appbox');
			...
			$GLOBALS['prisma_tpl']->set_var($var);
			$GLOBALS['prisma_tpl']->pparse('out','navbar_footer');
		...
		function display_sidebox($appname,$menu_title,$file)
		{
			if(!$appname || ($appname==$GLOBALS['phpgw_info']['flags']['currentapp'] && $file))
			{
				...
				$GLOBALS['prisma_tpl']->set_var($var);
				$GLOBALS['prisma_tpl']->pfp('out','extra_blocks_header');
		...
		function sidebox_menu_item($item_link='',$item_text='')
		{
			...
			{
				$GLOBALS['prisma_tpl']->pparse('out','extra_block_spacer');
			}
			else
			{
				...
				$GLOBALS['prisma_tpl']->pparse('out',$block);
			}
		}
		...
		function parse_navbar_end()
		{
			...
			$var = Array(
				'img_root'       => $GLOBALS['phpgw_info']['server']['webserver_url'] . '/phpgwapi/templates/prisma/images',
			...
			$GLOBALS['prisma_tpl']->set_var($var);
			$GLOBALS['prisma_tpl']->pfp('out','footer');
		}
