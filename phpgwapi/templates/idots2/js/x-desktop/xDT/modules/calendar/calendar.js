...
Calendar.cellClick = function(el, ev) {
	...
	if (typeof el.navtype == "undefined") {
		...
		switch (el.navtype) {
			...
			xDT.hide(xDT.window.name(window.name));
			xDT.alert(text.replace(/\n/g,"<br>"),"xDT.show('" + xDT.window.name(window.name) + "')"); //hack by xDT
			return;
		    case -2:
			if (year > cal.minYear) {
...
Calendar.prototype.create = function (_par) {
	...
	this.title = hh("", --title_length, 300); //hack by xDT
	hh("x", 1, 200).ttip = Calendar._TT["CLOSE"]; //hack by xDT
	this.title.className = "title";
	if (this.isPopup) {
		this.title.ttip = Calendar._TT["DRAG_TO_MOVE"];
		this.title.style.cursor = "move";
		//hh("&#x00d7;", 1, 200).ttip = Calendar._TT["CLOSE"]; //hack by xDT
	}
	row = Calendar.createElement("tr", thead);
...
Calendar.prototype.showAtElement = function (el, opts) {
	...
	this.element.style.display = "block";
	Calendar.continuation_for_the__khtml_browser = function() {
