...
class dragdrop
{
	...
	*/
	function validateBrowser()
	{
		...
		foreach(array('MOZILLA','IE') as $id=>$validatedBrowser)
		{
			if($this->browserTestMode || $clientBrowser->get_agent() == $validatedBrowser)
			{
				//error_log(__METHOD__.__LINE__.' B:'.$clientBrowser->get_agent().' V:'.(int)$clientBrowser->get_version());
				if ($validatedBrowser=='IE' && (int)$clientBrowser->get_version()<10) return false;
