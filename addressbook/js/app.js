...
class AddressbookApp extends EgwApp {
  ...
  push_grant_fields = ["owner", "shared_with"];
  push_filter_fields = ["tid", "owner", "cat_id"];
  /**
   * Constructor
   *
    ...
    switch (name) {
      case 'addressbook.edit':
        ...
        if (typeof content.showsearchbuttons == 'undefined' || !content.showsearchbuttons) {
          ...
          this.show_custom_country(jQuery('select[id*="adr_two_countrycode"]').get(0)); // Instanciate infolog JS too - wrong app, so it won't be done automatically
  ...
  observer(_msg, _app, _id, _type, _msg_type, _links) {
    ...
    if (_app === 'addressbook' && state && state.type && state.type === 'view' && state.id === _id) {
      ...
      if (content.data) {
        ...
        if (view) {
          view.widgetContainer._children[0].set_value({
  ...
  push(pushData) {
    ...
    if (pushData.app === 'stylite' && pushData.acl.missed && et2_id && et2_id.substr(0, 17) === 'addressbook-view-' && pushData.acl.account_id == this.egw.user('account_id') && pushData.acl.contact_id == this.et2.getArrayMgr("content")?.getEntry("id")) {
  ...
  view(_action, _senders) {
    ...
    extras.title = _action.id.match(/\-organisation/) && data.org_name != "" ? data.org_name : data.n_fn + " (" + egw.lang(extras.crm_list) + ")";
  ...
  */
    ...
    let callback = function (button, values) {
      if (button == et2_dialog.OK_BUTTON) {
        egw.json('addressbook.addressbook_ui.ajax_set_list', [0, values.name, values.owner, contacts], function (result) {
          ...
          if (result) {
            lists.options.select_options.unshift({
              value: result,
              label: values.name
            });
            lists.set_select_options(lists.options.select_options); // Set to new list so they can see it easily

            lists.set_value(result); // Call change event manually after setting the value
  ...
  adb_mail_vcard(_action, _elems) {
    ...
    if (fetchAll(_elems, nm, jQuery.proxy(function (ids) {
      this.adb_mail_vcard(_action, ids.map(function (num) {
        return {
          id: 'addressbook::' + num
        };
      }));
  ...
  addEmail(action, selected) {
    ...
    switch (action.id) {
      ...
      case "add_to_cc":
        egw.open_link('mailto:' + '?cc=' + emails.join(',').replace(/&/g, '__AMPERSAND__')); //egw.mailto('mailto:');
  ...
  setState(state, template) {
    var current_state = this.getState(); // State should be an object, not a string, but we'll parse
    ...
    if (current_state.app && current_state.id && (typeof state.state == 'undefined' || typeof state.state.app == 'undefined')) {
      ...
      if (index && index.widgetContainer) {
        ...
        if (options && (options.find && !options.find(function (e) {
          console.log(e);
          return e.value === state.state.grouped_view;
          ...
          return nm_action(action, senders, {}, {
            ids: [state.state.grouped_view]
          });
        }
      }
  ...
  geoLocationUrl(_dest_data, _dest_type, _src_data, _src_type) {
    ...
    {
      id: 'r0',
      val: _src_type === 'browser' ? _src_data.latitude : _src_data['adr_' + _src_type + '_street']
    }, {
      id: 't0',
      val: _src_type === 'browser' ? _src_data.longitude : _src_data['adr_' + _src_type + '_locality']
    }, {
      id: 'c0',
      val: _src_type === 'browser' ? '' : _src_data['adr_' + _src_type + '_countrycode']
    }, {
      id: 'z0',
      val: _src_type === 'browser' ? '' : _src_data['adr_' + _src_type + '_postalcode']
    }], [// destination address
    {
      id: 'r1',
      val: _dest_data['adr_' + dest_type + '_street']
    }, {
      id: 't1',
      val: _dest_data['adr_' + dest_type + '_locality']
    }, {
      id: 'c1',
      val: _dest_data['adr_' + dest_type + '_countrycode']
    }, {
      id: 'z1',
      val: _dest_data['adr_' + dest_type + '_postalcode']
    }]];
