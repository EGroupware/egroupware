...
describe('Et2EmailTag', () =>
{
	...
	afterEach(() =>
	{
		sinon.restore();
		window.egw.open = () => {};
		window.egw.jsonq = () => Promise.resolve({});
	});
	...
	{
		component.value = "unknown-plus@example.invalid";
		component.egw().jsonq = () => Promise.resolve({});
		const openSpy = sinon.spy();
		window.egw.open = openSpy;
		component.shadowRoot.querySelector("et2-button-icon").dispatchEvent(new MouseEvent('click'));
		assert.isTrue(openSpy.calledWithMatch('', 'addressbook', 'add',
			sinon.match({'presets[email]': 'unknown-plus@example.invalid'})));
	});
	...
	it('should open addressbook CRM on avatar click', async() =>
	{
		...
		component.value = 'known-contact@example.invalid';
		component.egw().jsonq = () => Promise.resolve({
			"known-contact@example.invalid": contact
		});
		const openSpy = sinon.spy();
		window.egw.open = openSpy;
		await component.handleContactMouseDown(new MouseEvent('click'));
		await new Promise(resolve => setTimeout(resolve));
		assert.isTrue(openSpy.calledWithMatch(contact.id, 'addressbook', 'view',
			sinon.match({title: contact.n_fn, icon: contact.photo})));
	});
});
