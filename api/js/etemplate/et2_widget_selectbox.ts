...
export class et2_selectbox extends et2_inputWidget
{
	...
	constructor(_parent, _attrs? : WidgetConfig, _child? : object)
	{
		...
		{
			if(isNaN(<number><unknown>this.options.rows))
			{
	...
	set_multiple(_multiple, _size)
	{
		...
		if (this.input)
		{
			if (_multiple)
			{
				...
				if ((<HTMLSelectElement>this.input[0]).options.length && (<HTMLSelectElement>this.input[0]).options[0].value === '')
				{
					(<HTMLSelectElement>this.input[0]).options[0] = null;
				}
			}
			else
			{
	 ...
	 */
	readonly _is_multiple_regexp = /^[,0-9A-Za-z/_ -]+$/;

	/**
	 * Regular expression and replace value for escaping values in jQuery selectors used to find options
	 */
	readonly _escape_value_replace = /\\/g;
	readonly _escape_value_with = '\\\\';
		...
		if(this.input != null && (this.options.search || this.options.tags) && !this.options.disabled)
		{
			...
			if (this.getType() == 'select-country') {
				...
				if (selected && selected.length == 1 && this.getValue())
				{
					...
					selected.find('span.img').remove();
					selected.prepend('<span class="img"></span>');
					selected.addClass('et2_country-select flag-'+ (<string>this.getValue()).toLowerCase());
				}
			}
	...
	set_select_options(_options) {
		...
		for (var key in _options)
		{
			...
			if (typeof _options[key] === 'object' && _options[key] !== null)
			{
				...
				{
					var label = isNaN(<number><unknown>key) ? key : _options[key].value;
	...
	isDirty()
	{
		if(this.input == null)
		{
			...
			// Array comparison
			return !(jQuery(this._oldValue).not(<string>value).length == 0 && jQuery(value).not(this._oldValue).length == 0);
		}
 ...
 */
export class et2_selectbox_ro extends et2_selectbox implements et2_IDetachedDOM
{
	optionValues: any = {};
	span: JQuery;
	/**
...
export class et2_menulist extends et2_DOMWidget
{
	...
	getDOMNode(_sender)
	{
		if (_sender != this)
		{
			return (<et2_DOMWidget>this.getParent()).getDOMNode(this);
		}
