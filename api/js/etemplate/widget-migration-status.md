...
| `file` | `et2_widget_file.ts` | Yes (`et2-file`) | 1b-kept-dependency | Real, ~780-line impl. `et2_widget_vfs.ts`'s `et2_vfsUpload extends et2_file` (registers `vfs-upload`) is the sole remaining reason this file stays - the reverse-direction `et2_vfsSize` import is gone (see `vfs-size` below). `et2_vfsUpload` itself has no external importers by name, so this is really a self-contained `et2_widget_file.ts` ↔ `et2_widget_vfs.ts` pair, not blocked by anything outside them - not yet investigated whether `file`'s other real-looking consumers (`mail/js/compose.ts`, `filemanager/js/filemanager.ts`, `et2_extension_customfields.ts`, `Et2Link/Et2LinkTo.ts`) are real or false positives. |
...
| `vfs-upload` | `et2_widget_vfs.ts` | Yes (`et2-vfs-upload`) | 1c-orphaned | `et2_vfsUpload extends et2_file` (legacy), real impl; `Et2VfsUpload extends Et2File` (webcomponent) is fully independent. Preprocessor-rewritten unconditionally, zero external importers of the legacy class by name - the only thing keeping it alive is its own `extends et2_file`, which is also why `et2_widget_file.ts`/`file` stays 1b (see there). **2026-09-04**: `_addFile()`'s own internal `et2_createWidget('vfs', ...)` call (a real, easy-to-miss programmatic dependency on the now-deleted `vfs` type) had to be fixed to `'vfs-path'` when `vfs` was deleted - same class of bug as the button/buttononly `et2_createWidget()` call sites. |
...
| 1a — shim | 1 |
| 1a — generated | 25 |
| 1b — kept for a dependent | 2 |
| 1c — orphaned legacy | 7 |
