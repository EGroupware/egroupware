...
| `textbox` | `et2_widget_textbox.ts` | Yes (`et2-textbox`) | 1b-kept-dependency | Real, large impl. Imported by legacy `et2_widget_number.ts` (`et2_number extends et2_textbox`), which itself is very much alive - see the `int`/`integer`/`float` note above. Also blocked independently by `hidden` (see next row) until 2026-09-03. |
| `hidden` | `et2_widget_textbox.ts` | Yes (`et2-hidden`) | 1c-orphaned | **Fixed 2026-09-03**: `<et2-textbox type="hidden">` still built (and merely CSS-hid) a full Shoelace `sl-input` - wasteful and never intended, same underlying issue as `int`/`integer`/`float` misusing `et2-textbox`. Added a genuine `Et2Hidden` webcomponent (`Et2Textbox/Et2Hidden.ts`, `Et2InputWidget(LitElement)` - no label/help-text/form-control chrome, just a real `<input type="hidden">` in its shadow DOM) and taught the preprocessor to rewrite both the bare `<hidden>` tag (added to `ADD_ET2_PREFIX_LEGACY_REGEXP`) and `<et2-textbox type="hidden">` (new dedicated regex pass, alongside the `int`/`integer`/`float` one) to it. `hidden` is now provably unreachable as a legacy type - moot for `et2_widget_textbox.ts` itself though, since `textbox`/`textbox_ro` (real 1b dependents) keep the file regardless. |
...
| `int`, `integer`, `float`, `old-int` | `et2_widget_number.ts` | Yes (`et2-number`) | 1c-orphaned | `et2_number extends et2_textbox`, real impl; `Et2Number` independent. `old-int` was live in `smallpart/templates/default/student.index.xet`, fixed 2026-09-03 (see `old-box` section note above) - zero usage anywhere now. The bare `<int>`/`<integer>`/`<float>` tags are preprocessor-rewritten unconditionally, but that check missed a second path: `Et2Widget.createElementFromNode()` (`Et2Widget/Et2Widget.ts:979-980`) lets a `type=` **attribute** win over the tag name, so `<et2-textbox type="integer">` (never touched by the bare-tag rewrite, since it's already `et2-`-prefixed) still routed to legacy `et2_number` via `et2_registry["integer"]` - confirmed live in ~30 templates incl. `calendar/templates/default/edit.xet`'s `quantity` field. **Fixed 2026-09-03**: new preprocessor pass rewrites `<et2-textbox type="int(eger)?|float">` to `<et2-number precision=.../>` too (`api/etemplate.php`, next to the bare-tag rewrite). With all three escape hatches now closed and no remaining real (non-type-only) code consumer found, `et2_widget_number.ts` may be a genuine 1a-generated candidate - **not yet acted on**, given this exact blind spot (type-attribute vs. tag-name resolution) was only just discovered and could have other instances not yet checked. |
