...
| `vfs` | `et2_widget_vfs.ts` | No | 2-not-migrated | Real base impl (file-attribute display); no plain `et2-vfs` tag exists (only prefixed sub-widgets, see below). **Checked 2026-09-03 as a possible 1a-generated candidate: no** - same story as `html`: `vfs` was never added to any preprocessor rewrite list at all (not a blind spot, just unmigrated), and genuinely-live bare `<vfs>` tags exist (`api/templates/default/vfsSelectUI.xet`, `filemanager/{config,home.rows,select}.xet`, `ranking`, `stylite`). Also has no webcomponent to redirect to in the first place. Good candidate for an actual `Et2Vfs` webcomponent implementation as a future project, not a shim-and-delete. |
...
| `vfs-mode` | `et2_widget_vfs.ts` | Yes (`et2-vfs-mode`) | 1c-orphaned | **Fixed 2026-09-03**: `Et2VfsMode` independent, but `vfs-mode` wasn't in `ADD_ET2_PREFIX_LEGACY_REGEXP` (unlike sibling `vfs-upload`) despite live usage (`filemanager/templates/default/home.rows.xet`) - same forgotten-entry pattern as `iframe`, silently dead code until now. Added to the rewrite list; `Et2VfsMode` gets its first real traffic. File stays regardless since `vfs`/`vfs-size` (other types in the same file) are still fully legacy and have no webcomponent to redirect to (see `vfs` above) - this is a correctness fix, not a step toward deleting `et2_widget_vfs.ts`. |
...
| 1a — shim | 1 |
| 1a — generated | 21 |
| 1b — kept for a dependent | 4 |
| 1c — orphaned legacy | 8 |
