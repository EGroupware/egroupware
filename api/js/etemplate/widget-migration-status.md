...
| `textbox` | `et2_widget_textbox.ts` | Yes (`et2-textbox`) | 1b-kept-dependency | Real, large impl. `et2_widget_number.ts` (formerly a dependent) was deleted 2026-09-03 - not what keeps this file alive any more. Still has real app-level importers though (`stylite`, `ranking`, `smallpart`, `filemanager`, `timesheet` `js/app.ts`/`filemanager.ts`), not yet individually checked for type-only-vs-real usage - not investigated further as part of the `et2_widget_number.ts` work. |
