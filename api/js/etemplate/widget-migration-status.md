...
| `html` | `legacy-shims/et2_widget_html.d.ts` (generated) | Yes (`et2-html`) | 1a-generated | Was: real impl (dozens of live templates - `mail/display.xet`, `infolog/index.xet`, `tracker/edit.xet`, ...). **Implemented 2026-09-04**: built a real `Et2Html` webcomponent (`Et2Html/Et2Html.ts`) - renders into the light DOM (`createRenderRoot()` returns `this`, same technique as `Et2Image`/`Et2VfsMime`/`Et2Customfields`) so arbitrary provided HTML/CSS inherits normal page styling rather than being shadow-DOM-isolated; replicates legacy's embedded-`<script>` execution (lit's `unsafeHTML()` inserts scripts inertly like plain `innerHTML` - each `<script>` element is recreated after render to force execution, verified by a real test); implements `et2_IDetachedDOM` and a `set_value()` alias for legacy content-array binding. Preprocessor: added `html` to `ADD_ET2_PREFIX_LEGACY_REGEXP` (was never in *any* rewrite list before, unlike `int`/`textbox`/etc. - not a blind spot, just never migrated). Real-consumer sweep found `mail/js/app.ts`'s dead `getMsg()` reaching into the legacy-only `.htmlNode` property (fixed to use `.value`; the method itself has zero real callers anywhere, matching this session's other "already-dead" patterns) - live-verified message-body/list rendering shows no other consumer relies on `.htmlNode`. Live-verified the widget itself works correctly (a real `id="css"` field in tracker rendering server-injected CSS, no console errors beyond the same generic "legacy grid could not handle adding child" warning already seen for other migrated widgets in this session). **Follow-up resolved 2026-09-04**: the flagged caveat turned out to be a real, live-confirmed break, not just a theoretical risk - SmallPart's "Add text" video-overlay feature threw `TypeError: Illegal constructor` (a registered custom element can only be constructed via `document.createElement()`, never legacy `et2_createWidget()`'s bare `new`). Fixed by converting `et2_smallpart_overlay_html(_editor)` to real `customElements.define()`'d webcomponents extending `Et2Html`/`Et2HtmlArea` - see `smallpart` repo commit "Fix HTML overlay editor: convert to real webcomponents" for full detail (also fixed a second bug found along the way: `Et2Widget.destroy()` doesn't reciprocally remove itself from its parent's children array, leaking stale editor references on every save/cancel/delete). Live-verified end-to-end: create → type → save (real server round-trip) → render. |
...
| `htmlarea_ro` | *(deleted, no `.d.ts`)* | Yes (`et2-htmlarea_ro`) | 1a-generated | Was: same class as `html` (`et2_register_widget(et2_html, ["html","htmlarea_ro"])`), `Et2HtmlAreaReadonly` already independent. **Checked 2026-09-04**: already fully unreachable *before* this deletion too - no bare `<htmlarea_ro>` tag or `type="htmlarea_ro"`/`type="htmlarea"` attribute anywhere in any `.xet`, and no programmatic `et2_createWidget('htmlarea'|'htmlarea_ro', ...)` call anywhere in the codebase (the only way the old `et2_registry["htmlarea_ro"]` redirect could ever have been reached, since `<htmlarea>` itself is unconditionally preprocessor-rewritten). Deleted alongside `html`. |
...
| `et2_widget_html.ts` | `html`, `htmlarea_ro` | `Et2Html` (`Et2Html/Et2Html.ts`, new webcomponent) |
