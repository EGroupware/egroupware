...
export class etemplate2
{
	...
	public static async handle_load(_type, _response)
	{
		...
		if(typeof data.url == "string" && typeof data.data === 'object')
		{
			...
			if(dialog && et2 !== null)
			{
				// Block dialog close if dirty
				dialog.addEventListener("sl-request-close", async(e) =>
				{
					...
					const unload = new CustomEvent("beforeunload", {cancelable: true});
					et2._close_changed_prompt(unload);
					if(unload.defaultPrevented)
					{
						// The actual prompt is normally generated by the browser for popups, but not here
						if(!confirm(egw.lang("changes that you made may not be saved.")))
						{
							e.preventDefault();
						}
					}
