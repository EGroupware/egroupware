...
class Select extends Etemplate\Widget
{
	...
	public function validate($cname, array $expand, array $content, &$validated=array())
	{
		...
		if (!$this->is_readonly($cname, $form_name))
		{
			...
			$type_options = self::typeOptions(
				...
				$false, $false, $value_in, $form_name
			);
	...
	public static function typeOptions($widget_type, $_legacy_options, &$no_lang=false, $readonly=false, &$value=null, $form_name=null)
	{
		...
		{
			$widget = $widget_type;
			$widget_type = $widget->attrs['type'] ?? $widget->type;
			if(substr($widget_type, 0, 4) == 'et2-')
			{
		...
		switch ($widget_type)
		{
			...
			case 'select-cat':
				...
				$application = $widget->attrs['application'] ?? $widget->getElementAttribute($form_name, 'application');
				$application = self::expand_name($application, 0, 0, '', '', self::$cont) ?? $type3;
				$globalCategories = $widget->attrs['globalCategories'] ?? $widget->getElementAttribute($form_name, 'globalCategories');
				...
				$parentCat = $widget->attrs['parentCat'] ?? $widget->getElementAttribute($form_name, 'parentCat');
