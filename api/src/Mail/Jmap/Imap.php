...
class Imap extends Jmap\Base
{
	...
	/**
	 * Parse an EXISTING blob's raw bytes AS A STANDALONE MESSAGE, returning JMAP-shaped Email
	 * properties (from/to/cc/subject/date/textBody/htmlBody/bodyValues/attachments) - no live
	 * IMAP/JMAP fetch needed once the raw bytes are in hand.
	 * doc/ai/projects/mail-compose-jmap-migration.md's follow-up (2026-08-31, ralf: "go ahead with
	 * that server-side blob-parse approach") - JMAP has no "parse this blob as if it were a real
	 * Email" verb (Email/get only ever works on a real, listed email id in a real mailbox), so a
	 * nested message/rfc822 ATTACHMENT (eg. a bounce/NDM's own original message, or any .eml nested
	 * inside another message) had no way to be viewed at all: mail_ui::displayMessage()'s own
	 * classic body-loading path (MessageDisplayHandler::get_load_email_data()) needs a real numeric
	 * IMAP UID, which a raw IMAP EMAILID search can't always resolve reliably against Stalwart (the
	 * exact "20s timeout, empty body" symptom found live getting here). Both backends are already
	 * fully CAPABLE of parsing arbitrary RFC822 content (that's what they do internally for every
	 * message in a mailbox already) - this just exposes that as its own read-only operation,
	 * reusing Horde_Mime_Part::parseMessage() directly (same primitive
	 * ImapBuildMailerTest.php's own tests already parse a raw message back with, and unlike
	 * emailBodyFields()'s IMAP-FETCH-coupled version, parseMessage() gives BOTH headers and fully
	 * transfer-decoded body content from the raw text in one pass - no per-part IMAP FETCH needed).
	 *
	 * Attachments WITHIN the parsed message are listed (name/type/size/cid/disposition) but not
	 * independently downloadable - their own blobId is a synthetic "parsed:" reference this class
	 * does not (yet) resolve back to bytes. A narrow, deliberate limitation for this first version,
	 * not a bug - the common case (viewing the nested message's own header/body) works fully.
	 *
	 * @param string $accountId
	 * @param string $blobId the ALREADY-KNOWN blobId of a message/rfc822 part (from a real
	 *  Email/get's own attachments listing) - never a bare mailbox/uid/partId string constructed
	 *  by the caller itself
	 * @return ?array {from, to, cc, subject, date, textBody, htmlBody, bodyValues, attachments} or
	 *  null if the blob can't be resolved to bytes at all
	 */
	public static function parseBlobAsEmail(string $accountId, string $blobId) : ?array
	{
		$raw = \EGroupware\Mail\Ui\AttachmentJmap::fetchBlobBytes($accountId, $blobId);
		if ($raw === null)
		{
			return null;
		}
		return self::parseRawMessageAsEmail($raw, $blobId);
	}

	/**
	 * Pure parsing half of parseBlobAsEmail() above (raw bytes in, JMAP-shaped Email properties
	 * out, no I/O at all) - split out purely so it's independently unit-testable without needing a
	 * live IMAP/JMAP connection to exercise AttachmentJmap::fetchBlobBytes() itself (see
	 * ImapBuildMailerTest.php, which already parses a raw message back the same way via
	 * Horde_Mime_Part::parseMessage()).
	 *
	 * @param string $raw raw RFC822 message bytes
	 * @param string $sourceBlobId the ORIGINAL blob's own id - only used to build a synthetic
	 *  "parsed:" blobId prefix for any attachments WITHIN this parsed message (see this method's
	 *  own class-level docblock for why those aren't independently downloadable yet)
	 * @return array {from, to, cc, subject, date, textBody, htmlBody, bodyValues, attachments}
	 */
	public static function parseRawMessageAsEmail(string $raw, string $sourceBlobId) : array
	{
		$headers = \Horde_Mime_Headers::parseHeaders($raw);
		$structure = \Horde_Mime_Part::parseMessage($raw, ['forcemime' => true]);

		$textId = $structure->findBody('plain');
		$htmlId = $structure->findBody('html');
		$bodyValues = [];
		$attachments = [];
		// deliberately NOT $structure->partIterator() (a flat walk) - unlike a live IMAP
		// BODYSTRUCTURE fetch (which never descends into a message/rfc822 part's OWN internal MIME
		// structure), parseMessage() parses the COMPLETE raw bytes, including whatever is INSIDE a
		// nested message/rfc822 attachment - a manual recursive walk that only ever descends into
		// 'multipart' parts (never into a message/rfc822 leaf's own children) treats a nested
		// message as ONE opaque attachment, matching what a real Email/get's own `attachments`
		// property already looks like for the SAME structure fetched live via IMAP.
		$walk = function(\Horde_Mime_Part $part) use (&$walk, &$bodyValues, &$attachments, $textId, $htmlId, $sourceBlobId)
		{
			$id = $part->getMimeId();
		...
		return [
			'from' => self::addressListFromHeader($headers, 'From') ?? [],
			'to' => self::addressListFromHeader($headers, 'To') ?? [],
			'cc' => self::addressListFromHeader($headers, 'Cc') ?? [],
			'subject' => (string)($headers['subject']->value ?? ''),
			'date' => (string)($headers['date']->value ?? ''),
			'textBody' => $textId !== null ? [['partId' => $textId, 'type' => 'text/plain']] : [],
			'htmlBody' => $htmlId !== null ? [['partId' => $htmlId, 'type' => 'text/html']] : [],
			'bodyValues' => $bodyValues,
			'attachments' => $attachments,
		];
	}
