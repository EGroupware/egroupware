 ...
 * `$session->mailbox`, `$session->email`), each an instance of a `Type` subclass declared by
 ...
 * HTTP-specific (`Api\Jmap`, the real-JMAP-over-HTTP concrete subclass) - both are concrete
 * subclasses of this class, not part of its contract.
 *
 * Named `Base`, not the bare `Jmap`, matching this codebase's own `Api\Storage`/`Api\Storage\Base`
 * convention: the concrete, actually-used class (`Api\Jmap`, real-JMAP-over-HTTP) lives directly
 * under `Api\`, where you look first for something usable - this abstract base lives one level
 * down instead (ralf, 2026-08-27).
 */
abstract class Base
{
	/**
	 * type-name (eg. "mailbox") => concrete Type subclass FQCN, declared by each concrete
	...
	/**
	 * Lazily-instantiated, memoized Type objects, keyed the same as $types
	 *
	 ...
	 *
	 * @param string $name
	 * @return Type|null null if this session has no such type
	 */
	 ...
	 *
	 * Only meaningful for a session that genuinely speaks JMAP-over-something (eg. `Api\Jmap`,
	 * real HTTP) - `Type`'s default get()/query()/set() implementations call this. A session
