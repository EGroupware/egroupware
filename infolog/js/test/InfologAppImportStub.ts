/**
 * Globals that have to exist BEFORE infolog/js/app.ts (and the api/ import chain it pulls in) is
 * evaluated - import this module ahead of "../app", ESM evaluates imports in declaration order.
 *
 * Mirrors addressbook/js/test/AddressbookAppImportStub.ts, mail/js/test/MailAppImportStub.ts and
 * calendar/js/test/CalendarAppImportStub.ts - the requirements are not app-specific, they come
 * from egw.js's bootstrap and the etemplate2/Et2* import chain every app.ts pulls in:
 * - `app.classes`: app.ts registers itself with `app.classes.infolog = InfologApp` at module scope.
