 ...
 * own hand-rolled code used to duplicate) rather than re-implemented.
 *
 * This class does NOT override search() - phase 3 of the migration doc moved the whole InfoLog
 * search assembly (ACL fold-in, status/date/category/responsible/action-link filtering,
 * free-text/RAG search) to infolog_bo::searchInfolog(), which builds the generic
 * $criteria/$filter/$join/$order_by arguments and calls the plain inherited
 * Api\Storage::search() via $this->so - the "override search(), call parent::search()"
 * convention most Api\Storage-based apps use, just built by the BO layer instead of an
 * override here, since infolog_bo needs $query by reference for its start/total contract (an
 * override of search()'s by-value signature can't provide that). Custom-field col_filter/
 * order_by ("#name") entries flow straight through to cf_filter()/order_by_cf(), invoked
 * automatically by the inherited process_search() - no manual delegation call needed any more.
 *
