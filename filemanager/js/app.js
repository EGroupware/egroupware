import { E as EgwApp, a as etemplate2, e as egw, b as et2_dialog, y as et2_file, z as et2_button, A as et2_nextmatch_controller, w as egw_get_file_editor_prefered_mimes, d as et2_createWidget } from '../../chunks/etemplate2-0eb045cf.js';
...
class filemanagerAPP extends EgwApp {
  ...
  constructor() {
    // call parent
    super('filemanager'); // Loading filemanager in its tab and home causes us problems with
  ...
  et2_ready(et2, name) {
    ...
    if (path_widget) // do NOT set not found path-widgets, as uploads works on first one only!
      {
        this.path_widget[et2.DOMContainer.id] = path_widget; // Bind to removal to remove from list
    ...
    this.clipboard_tooltips(); // calling set_readonly for initial path
  ...
  setState(state) {
    ...
    let result = super.setState(state, 'filemanager.index'); // This has to happen after the parent, changing to tile recreates
    // nm controller
  ...
  open_mail(attachments, params) {
    ...
    content.data.files["filemode"] = params['preset[filemode]']; // always open compose in html mode, as attachment links look a lot nicer in html
  ...
  upload(_event, _file_count, _path, _conflict = "ask", _target = 'filemanager_ui::ajax_action') {
    ...
    if (_file_count && !jQuery.isEmptyObject(_event.data.getValue())) {
      ...
      }], this._upload_callback, this, true, this).sendRequest();
      widget.set_value('');
    }
  }
  /**
  ...
  file_a_file_upload(_event, _file_count) {
    ...
    egw.json('filemanager_ui::ajax_action', [action == 'save_as' ? 'upload' : 'link', widget.getValue(), path, props], function (_data) {
      app.filemanager._upload_callback(_data); // Remove successful after a delay
  ...
  _upload_callback(_data) {
    ...
    for (let file in _data.uploaded) {
      if (_data.uploaded[file].confirm && !_data.uploaded[file].confirmed) {
        ...
        if (_data.uploaded[file].confirm === "is_dir") buttons.shift();
        let dialog = et2_dialog.show_prompt(function (_button_id, _value) {
          ...
          switch (_button_id) {
            ...
            case "rename":
              ...
              delete uploaded[this.my_data.file].confirm; // send overwrite-confirmation and/or rename request to server
        ...
        }, _data.uploaded[file].confirm === "is_dir" ? this.egw.lang("There's already a directory with that name!") : this.egw.lang('Do you want to overwrite existing file %1 in directory %2?', _data.uploaded[file].name, _data.path), this.egw.lang('File %1 already exists', _data.uploaded[file].name), _data.uploaded[file].name, buttons, file); // setting required data for callback in as my_data
  ...
  clipboard(_action, _elems) {
    ...
    clipboard.type = jQuery.unique(clipboard.type); // egwAction is a circular structure and can't be stringified so just take what we want
    // Hopefully that's enough for the action handlers

    for (let k in _elems) {
      if (_elems[k].id) clipboard.selected.push({
        id: _elems[k].id,
        data: _elems[k].data
      });
    } // Save it in session
    ...
    et2_dialog.show_prompt(function (button, dir) {
      if (button && dir) {
        ...
        if (action && action instanceof egwAction) {
          ...
          if (paths[0]) path = paths[0]; // check if target is a file --> use it's directory instead
  ...
  force_download(_action, _senders) {
    for (let i = 0; i < _senders.length; i++) {
      ...
      let $a = jQuery(a).prop('href', url).prop('download', data ? data.data.name : "").appendTo(this.et2.getDOMNode());
  ...
  change_dir(_dir, widget) {
    ...
    if (widget) etemplate_name = widget.getInstanceManager().uniqueId; // Make sure everything is in place for changing directory

    if (!this.et2 || typeof etemplate_name !== 'string' || typeof this.path_widget[etemplate_name] === 'undefined') {
      return false;
    }
  ...
  change_view(view, button_widget) {
    ...
    nm.set_view(view); // Put it into active filters (but don't refresh)

    nm.activeFilters["view"] = view; // Change template to match

    let template = view == et2_nextmatch_controller.VIEW_ROW ? 'filemanager.index.rows' : 'filemanager.tile';
    nm.set_template(template); // Wait for template to load, then refresh
  ...
  open(_action, _senders) {
    ...
    let mime = this.et2._inst.widgetContainer.getWidgetById('$row'); // try to get mime widget DOM node out of the row DOM


    let mime_dom = jQuery(_senders[0].iface.getDOMNode()).find("span#filemanager-index_\\$row");
    let fe = egw_get_file_editor_prefered_mimes(); // symlinks dont have mime 'http/unix-directory', but server marks all directories with class 'isDir'
    ...
    if (data.data.mime == 'httpd/unix-directory' || data.data['class'] && data.data['class'].split(/ +/).indexOf('isDir') != -1) {
      ...
      let url; // Build ViewerJS url

      if (data.data.mime.match(/application\/vnd\.oasis\.opendocument/) && egw.preference('document_doubleclick_action', 'filemanager') == 'collabeditor') {
  ...
  paste_enabled(_action, _senders, _target) {
    ...
    actions.push({
      id: _action.id + '_current',
      caption: current_dir,
      path: current_dir,
      enabled: dir && dir.data && dir.data.class && dir.data.class.indexOf('noEdit') === -1 || !dir && path_widget && !path_widget.options.readonly
    }); // Target, if directory
    ...
    actions.push({
      ...
      caption: target_dir,
      path: target_dir,
      enabled: _target && _target.iface && jQuery(_target.iface.getDOMNode()).hasClass('isDir') && (dir && dir.data && dir.data.class && dir.data.class.indexOf('noEdit') === -1 || !dir)
    }); // Last 10 folders
    ...
    _action.updateActions(actions); // Create paste action
    ...
    let paste_exec = function (action, selected) {
      ...
      let clipboard = JSON.parse(egw.getSessionItem('phpgwapi', 'egw_clipboard')); // Set a flag so apps can tell the difference, if they need to

      action.set_onExecute(action.parent.onExecute.fnct);
      action.execute(clipboard.selected, selected[0]); // Clear the clipboard, the files are not there anymore
  ...
  drop(_action, _elems, _target) {
    let src = this._elems2paths(_elems); // Target will be missing ID if directory is empty
    ...
    let dst; // Action specifies a destination, target does not matter
    ...
    if (_action.data && _action.data.path) {
      dst = _action.data.path;
        ...
        if (paths[0]) dst = paths[0]; // check if target is a file --> use it's directory instead
    ...
    egw.set_preference(this.appname, 'drop_history', previous_dsts); // Actual action id will be something like file_drop_{move|copy|link}[_other_id],
  ...
  filedrop(row_uid, files) {
    ...
    let widget = this.et2.getWidgetById('upload'); // Override finish to specify a potentially different path
    ...
    widget.set_value(files); // Return false to prevent the link
  ...
  select_clicked(event, widget) {
    if (widget?.value?.is_dir) // true for "httpd/unix-directory" and "egw/*"
      {
        let path = null; // Cannot do this, there are multiple widgets named path
  ...
  _share_link_callback(_data) {
    ...
    et2_createWidget("dialog", {
      ...
      title: _data.title ? _data.title : _data.writable || _data.action === 'shareWritableLink' ? this.egw.lang("Writable share link") : this.egw.lang("Readonly share link"),
      template: _data.template,
      width: 450,
      value: {
        content: {
          "share_link": _data.share_link
        }
      }
    });
  }
  ...
  hidden_upload_enabled(_action, _senders) {
    ...
    let data = egw.dataGetUIDdata(_senders[0].id);
    let readonly = (data?.data.class || '').split(/ +/).indexOf('noEdit') >= 0; // symlinks dont have mime 'http/unix-directory', but server marks all directories with class 'isDir'
  ...
  copy_link(_action, _senders) {
    ...
    if (url.substr(0, 4) == 'http' && url.indexOf('://') <= 5) {// it's already a full url
    } else {
