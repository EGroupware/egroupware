/**
 * Globals that have to exist BEFORE calendar/js/app.ts (and the api/ import chain it pulls in) is
 * evaluated - import this module ahead of "../app", ESM evaluates imports in declaration order.
 *
 * Mirrors mail/js/test/MailAppImportStub.ts - the requirements are not calendar-specific, they
 * come from egw.js's bootstrap and the etemplate2/Et2* import chain every app.ts pulls in:
 * - `app.classes`: app.ts registers itself with `app.classes.calendar = CalendarApp` at module scope.
 ...
 * - `egw.user`: calendar/js/et2_widget_view.ts reads egw.user('account_id') at module scope, for
 *   the owner attribute's default.
 */
