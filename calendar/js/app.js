...
var CalendarApp = /** @class */ (function (_super) {
    ...
    */
    ...
    CalendarApp.prototype.push_calendar = function (pushData) {
        ...
        var owner_check = et2_widget_event_1.et2_calendar_event.owner_check(cal_event, 
        ...
        var process_data = function (data) {
            ...
            _this._update_events(_this.state, [data.uid]);
        };
    ...
    CalendarApp.prototype._scroll = function () {
        ...
        var scroll_animate = function (direction, delta) {
            // Scrolling too fast?
            if (app.calendar._scroll_disabled)
                return;
            // Find the template
    ...
    CalendarApp.prototype.event_change = function (event, widget, dialog_button) {
        ...
        if (widget.options.value.app == 'infolog') {
            ...
            egw().json('stylite_infolog_calendar_integration::ajax_moveInfologEvent', [widget.options.value.app_id, widget.options.value.start, widget.options.value.duration], 
    ...
    CalendarApp.prototype.status = function (_action, _events) {
        ...
        for (var i = 0; i < _events.length; i++) {
            ...
            event_widget.recur_prompt(jQuery.proxy(function (button_id, event_data) {
                switch (button_id) {
                    case 'exception':
                        egw().json('calendar.calendar_uiforms.ajax_status', [event_data.app_id, egw.user('account_id'), _action.data.id]).sendRequest(true);
                        break;
                    case 'series':
                    case 'single':
                        egw().json('calendar.calendar_uiforms.ajax_status', [event_data.id, egw.user('account_id'), _action.data.id]).sendRequest(true);
                        break;
                    case 'cancel':
                    default:
                        break;
    ...
    CalendarApp.prototype.delete = function (_action, _events) {
        ...
        for (var i = 0; i < _events.length; i++) {
            ...
            event_widget.recur_prompt(jQuery.proxy(function (button_id, event_data) {
                switch (button_id) {
                    case 'exception':
                        egw().json('calendar.calendar_uiforms.ajax_delete', [event_data.app_id]).sendRequest(true);
                        break;
                    case 'series':
                    case 'single':
                        egw().json('calendar.calendar_uiforms.ajax_delete', [event_data.id]).sendRequest(true);
                        break;
                    case 'cancel':
                    default:
                        break;
    ...
    CalendarApp.prototype.update_state = function (_set) {
        ...
        if (this.state_update_in_progress)
            return;
        var changed = [];
        ...
        if (CalendarApp.views[state.state.view] &&
            ...
            */
            if (grid) {
                ...
                egw.loading_prompt(this.appname, true, egw.lang('please wait...'), typeof framework !== 'undefined' ? framework.applications.calendar.tab.contentDiv : false, egwIsMobile() ? 'horizontal' : 'spinner');
                var loading = false;
                var value = [];
                ...
                grid.iterateOver(function (widget) {
                    ...
                    if (widget.id == value[row_index].id &&
                        ...
                        for (var day = 0; was_disabled[row_index] && day < widget.day_widgets.length; day++) {
                            egw.dataStoreUID(widget.day_widgets[day].registeredUID, egw.dataGetUIDdata(widget.day_widgets[day].registeredUID).data);
                        }
    ...
    CalendarApp.prototype._need_data = function (value, state) {
        ...
        for (var i = 0; i < value.length; i++) {
            ...
            if (need_data && seperate_owners) {
                this._fetch_data(jQuery.extend({}, state, { owner: value[i].owner, selected_owners: state.owner }), this.sidebox_et2 ? null : this.et2.getInstanceManager());
                need_data = false;
            }
        }
    ...
    CalendarApp.prototype._fetch_data = function (state, instance, start, specific_ids) {
        ...
        if (cat_id && typeof cat_id == 'string' && cat_id != "0")
            cat_id = cat_id.split(',');
