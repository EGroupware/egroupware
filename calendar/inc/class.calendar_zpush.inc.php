...
class calendar_zpush implements activesync_plugin_write, activesync_plugin_meeting_requests
{
	...
	public function GetMessage($folderid, $id, $contentparameters, $class='SyncAppointment')
	{
		...
		ZLog::Write(LOGLEVEL_DEBUG, __METHOD__."($folderid,$id,...) start=".Api\DateTime::to($event['start'], 'Y-m-d H:i:s').", recurrence=".(!empty($event['recurrence'])?Api\DateTime::to($event['recurrence'], 'Y-m-d H:i:s'):''));
