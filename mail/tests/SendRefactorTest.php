<?php
/**
 * EGroupware Mail: regression test for the Compose::send() -> Send extraction
 *
 ...
 * -building logic it shares with Compose::saveAsDraft() (createMessage()/_getAttachmentLinks()/
 * resolveEmailAddressList()/convertHTMLToText()/_encrypt()/changeProfile()) moved into a shared
 * ComposeMessageBuilder trait instead of being duplicated or making Send depend on a full Compose
 * instance. mail_compose itself was later renamed/moved to EGroupware\Mail\Compose (mail/src/
 * Compose.php, git mv'd from mail/inc/class.mail_compose.inc.php to preserve blame) - this test
 * updated accordingly, still covering the same split.
 *
 ...
 * failure against the pre-existing, unmodified Compose constructor too - not something this
 ...
 * independent per-instance (not shared through the trait), Compose no longer has send() at
...
class SendRefactorTest extends Api\LoggedInTest
{
	public function testTraitPropertiesAreIndependentPerInstance()
	{
		...
		$this->assertNotSame($send->mailPreferences['sendOptions'], $compose->mailPreferences['sendOptions'],
			'Send and Compose must not share mailPreferences state through the trait');
	}

	public function testTraitCompositionShape()
	{
		$sendReflection = new ReflectionClass(Send::class);
		$composeReflection = new ReflectionClass(Compose::class);
					 ...
					 'convertHTMLToText', '_encrypt', 'changeProfile'] as $method)
		{
			...
			$this->assertTrue($composeReflection->hasMethod($method), "Compose::$method() missing");
		}

		$this->assertTrue($sendReflection->hasMethod('send'), 'Send::send() missing');
		$this->assertFalse($composeReflection->hasMethod('send'),
			'Compose::send() should be gone - moved to Send entirely, not shared via the trait');

		$this->assertTrue($composeReflection->hasMethod('saveAsDraft'),
			'Compose::saveAsDraft() should still exist, using createMessage() via the trait');
	}
