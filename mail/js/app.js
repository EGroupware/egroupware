...
app.classes.mail = AppJS.extend(
    {
        ...
        notifyNew: function (pushData) {
            ...
            if (typeof notify === 'undefined' || notify === 'always' ||
                ...
                this.egw.notification(egw.lang('new mail'), {
                    body: message,
                    tag: 'mail',
                    icon: egw.image('navbar', 'mail')
                });
            }
        },
        ...
        attachmentsBlockActions: function (_e, _widget) {
            ...
            _widget.label = this.egw.lang(_widget.select_options.filter(_item => {
        ...
        mail_preview: function (selected, nextmatch) {
            ...
            // Not applied to mobile preview
                ...
                for (var t in this.W_TIMEOUTS) {
                    window.clearTimeout(this.W_TIMEOUTS[t]);
                }
                this.W_TIMEOUTS.push(window.setTimeout(function () {

                    console.log(rowId);
                    // Request email body from server
                    IframeHandle.set_src(egw.link('/index.php', {
                        menuaction: 'mail.mail_ui.loadEmailBody',
                        _messageID: rowId
                    }));
        ...
        mail_setQuotaDisplay: function (_data) {
            ...
            if (quotabox) {
                ...
                if (_data.quotawarning) {
                    ...
                    var buttons = [
                        {
                            label: this.egw.lang("Empty Trash and Junk"),
                            id: "cleanup",
        ...
        mail_queueRefreshFolderList: function (_folders) {
            ...
            window.setTimeout(function () {
                egw.jsonq('mail.mail_ui.ajax_setFolderStatus', [_folders], function () {
                    self.unlock_tree();
                });
            }, 500);
        },
        ...
        mail_emptySpam: function (action, _senders) {
            ...
            egw.json('mail.mail_ui.ajax_emptySpam', [server[0], activeFilters['selectedFolder'] ? activeFilters['selectedFolder'] : null], function () {
                self.unlock_tree();
            })
                .sendRequest(true);
        ...
        mail_emptyTrash: function (action, _senders) {
            ...
            egw.json('mail.mail_ui.ajax_emptyTrash', [server[0], activeFilters['selectedFolder'] ? activeFilters['selectedFolder'] : null], function () {
                self.unlock_tree();
            })
                .sendRequest(true);
        ...
        mail_changeFolder: function (_folder, _widget, _previous) {
            ...
            if (server[0] != previousServer[0] && profile_selected) {
                ...
                return this.mail_changeProfile(_folder, _widget, _widget.getSelectedItem().item.length === 0);
            }
...
*/
            ...
            switch (action) {
                ...
                case 'saveAllToVfs':
                    ...
                    var vfs_select = et2_createWidget('vfs-select', {
                        ...
                        method_id: ids.length > 1 ? {ids: ids, action: 'attachment'} : {
                            ids: ids[0],
                            action: 'attachment'
                        },
        ...
        mail_callCopy: function (_action, _senders, _target, _allMessagesChecked) {
            ...
            if (messages['all']) messages['activeFilters'] = this.mail_getActiveFilters(_action);
            var self = this;
            egw.json('mail.mail_ui.ajax_copyMessages', [target, messages], function () {
                self.unlock_tree();
            })
                .sendRequest();
...
//			request.sendRequest();//false, this._upload_callback, this);
        ...
        folderManagement: function (_action, _senders) {
            var acc_id = parseInt(_senders[0].manager.data.widget.getSelectedItem().id);
            ...
            this.viewEntry(_action, _sender, true, function (etemplate) {
                ...
                jQuery(iframe.getDOMNode()).on('load', function () {
                    ...
                    if (jQuery(this.contentWindow.document.body).find('#calendar-meeting').length > 0) {
                        ...
                        window.setTimeout(function () {
                            jQuery(frame).height(frame.contentWindow.document.body.scrollHeight);
                        }, 500);
                    } else {
                        self.resolveExternalImages(this.contentWindow.document);
