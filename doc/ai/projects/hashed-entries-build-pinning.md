...
Scoping/design doc. **No code written yet** — the approach is agreed in outline (see
[Decisions made](#decisions-made)) but two decisions are still open and nothing should be
implemented until they are settled.
...
Scoped and decided; **not started, no code written**. All design decisions are settled — location
(`chunks/`), pinning granularity (per document), manifest delivery (`data-manifest` attribute), and
the `getImportMap()` cleanup split. Ready to implement against the breakdown in
[The work](#the-work). Get fresh confirmation before writing code.
