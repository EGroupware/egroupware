...
- **Done (2026-09-09)**: `MailApp.resolveAttachmentViewUrls()` - 8 tests,
  `mail/js/test/MailAppResolveAttachmentViewUrls.test.ts`. Replaces an attachment row's server
  `mime_url` with a client-side JMAP `blob:` object URL for the "click to view" path, skipping the
  classic server round-trip - a broken exclusion filter could route a `message/rfc822`
  sub-message or a vCard/iCalendar import (both need real server-side handling: a whole
  sub-message view, or a contact/event-creating side effect - never a blob render) through the
  wrong path. Covers: the non-JMAP-row no-op (never touches `jmap` at all), the
  no-eligible-attachments no-op, skipping items without a `blobId`, excluding
  `message/rfc822`/vCard/iCalendar case-insensitively even WITH a `blobId`, revoking previously-
  created view URLs before resolving new ones, per-item `mime_url` writes, one attachment's
  failure not blocking the others (partial success still reports overall success, the failed
  item's `mime_url` stays untouched), and total failure reporting `false` without throwing.
...
- **Done (2026-09-09)**: `Ui\FolderHandler::setFolderStatus()`'s guard clauses - 3 tests,
  `mail/tests/FolderHandlerSetFolderStatusTest.php`. `folderSubscription()` has no self-contained
  slice at all (immediately does real IMAP work) - not attempted. `setFolderStatus()`'s early
  return for JMAP accounts is already flagged fragile by its own docblock ("JMAP-FALLTHROUGH-GUARD"
  - a real live bug found 2026-08-24: without it, a JMAP account falls through to a raw-socket
  call that hangs/misconnects against a JMAP(S) endpoint) - confirmed this test actually catches a
  broken guard (temporarily disabled it, watched the test fail with the expected "must not be
  called" exception, restored it). Also covers the empty/null `$_folder` no-op. `new Ui(false)`
  skips the real IMAP/DB bootstrap entirely, `mail_bo` set directly to a throwing stub.
- **Done (2026-09-09)**: `Ui\ImportHandler::importMessageToFolder()`'s JMAP-native import path -
  6 tests, `mail/tests/ImportHandlerImportMessageToFolderTest.php`. Same
  "JMAP-FALLTHROUGH-GUARD" pattern as `FolderHandler::setFolderStatus()` but more substantial - a
  real live bug found 2026-09-03 (clicking an InfoLog-attached `.eml` on a Stalwart account failed
  with "Zielordner Drafts existiert nicht"): `folderExists()`/`appendMessage()` fall through to
  raw-socket methods, unguarded for a JMAP account, so this materializes the import via genuine
  `Email/import` (RFC 8621 §4.8) instead. Confirmed the tests catch a regression: temporarily
  disabled the guard, watched 5 of 6 tests fail with the expected "must not be called" exception
  from the stub (falling through to the classic path exactly like the real bug), restored it.
  `$_formData['file']` uses the `egw-data://` scheme throughout, so `Mail::checkFileBasics()`
  returns immediately with no real temp-file dependency. Covers: the JMAP-shaped rowId
  construction, the raw message bytes actually uploaded as the blob, `INBOX`+delimiter
  normalization before folder resolution, the "target folder doesn't exist" failure (never
  uploads a blob), any other JMAP failure wrapped into `Api\Exception\WrongUserinput` with the
  original message, and the empty-`$_folder` guard never touching JMAP at all.
- **Done (2026-09-09)**: `Ui\AttachmentHandler::getdisplayableBody()` - 9 tests,
  `mail/tests/AttachmentHandlerGetDisplayableBodyTest.php`. The last step before a message body is
  actually shown to the user - real risk across several dimensions at once: HTML sanitization (a
  broken script-tag-removal path would be an XSS hole, not just a display bug), entity encoding,
  and multi-part joining. Covers: the empty-input no-op, plain-text HTML-entity-encoding + `<pre>`
  wrapping, URL auto-linkification (and that `modifyURI=false` suppresses it while keeping the raw
  text), HTML `<script>` tags AND their content being completely removed (not just the tag
  markers), multi-part joining via the `<hr>` separator, a whitespace-only part contributing
  nothing (not even a stray separator), Windows-share backslash-path linkification, and recursing
  into a nested (multipart-shaped) body-parts list. `Tree.php` (`getAccountsRootNode()`/
  `getInitialIndexTree()`) was considered but abandoned - it needs real, live-configured mail
...
- 2026-09-09: `Ui\BodyHandler`'s inline (`cid:`) image resolution (10 tests,
  ...
  covered.
- 2026-09-09: `Ui\FolderHandler::setFolderStatus()`'s guard clauses (3 tests,
  `FolderHandlerSetFolderStatusTest.php`) done - see the delegate-classes entry above for what's
  covered, including confirming this test actually catches a broken JMAP-fallthrough guard.
- 2026-09-09: `Ui\ImportHandler::importMessageToFolder()`'s JMAP-native import path (6 tests,
  `ImportHandlerImportMessageToFolderTest.php`) done - see the delegate-classes entry above for
  what's covered, including confirming these tests actually catch a broken JMAP-fallthrough guard
  (5 of 6 fail when disabled).
