...
## Post-move incident (2026-09-08): stale JS bundle, not a missed call site

Right after the `Compose` rename above, ralf hit a live prod error reproducing it via "reply to a
mail open in preview": `CreateObject(mail_compose) file .../class.mail_compose.inc.php not found!`
(thrown from `api/src/loader/deprecated_factory.php`'s legacy `CreateObject()` fallback, invoked by
`Json\Request::handleRequest()` when `class_exists($className)` is false).

Root cause was **not** a missed call site - `mail/js/app.min.js` (and every app's own `app.min.js`)
is gitignored (`.gitignore:75`), never committed, and only regenerated by `npm run build` or an
active `npm run build:watch` file-watcher. The served bundle's mtime (Sep 7 17:49) predated the
rename's source edits (Sep 8 10:53) - it was still shipping the old `mail_compose` menuaction
strings. Confirmed by running `npm run build` (bundle mtime updated, "mail_compose" grep-count
inside it dropped 55→46, remainder all comments/dead regex, "EGroupware\\Mail\\Compose" now
present) and ralf separately restarting `build:watch`; both together resolved it ("replying works
now"). See [[feedback_js_bundles_gitignored_need_build]] - saved as a standing gotcha to check for
proactively in future sessions touching any app's `.ts` source in this shared checkout.

Follow-up pushback ("but grepping for mail_compose still finds some usages") was answered by
categorizing every remaining hit: inert comments/docblocks, or `COMPOSE_POPUP_URL_PATTERN`'s
now-dead `mail_compose\.compose` regex alternative (matches nothing, since that postback no longer
exists) - which led directly to the two cleanups below.

## Step 10 follow-up (2026-09-08): dead `mail_compose.compose` regex alternative removed

`COMPOSE_POPUP_URL_PATTERN` (and its 4 duplicated inline instances) - the regex
`egw.openWithinWindow()` uses to detect an already-open compose popup to reuse - still matched
`mail_compose\.compose` alongside `/mail/compose.php`, left over from before the classic postback
was deleted. Since no popup can carry that url any more, simplified to just
`/\/mail\/compose\.php/` in all 4 files: `mail/js/app.ts` (the shared `COMPOSE_POPUP_URL_PATTERN`
const + one nearby comment), `filemanager/js/filemanager.ts` (`open_mail()` + the share-link
builder), `addressbook/js/app.ts` (`adb_mail_vcard()`), `api/js/jsapi/egw_open.ts` (`mailto()`).
Verified: `npx tsc --noEmit` shows the same pre-existing errors before/after (none in these 4
files); `mail` jstest group 197/197, `addressbook` group 5/5 (filemanager has no `.test.ts` files
at all). One unrelated historical-prose comment in `egw_open.ts` (~line 826, describing why
`clientSidePopup()`... actually `open_link()`'s data-include handling differs for a
server-rendered popup) deliberately left as-is - it explains past behavior, isn't live regex code.
Committed as `1606d45830`.

## Step 10 follow-up (2026-09-08): ImportHandler's dead `composeFromDraft` menuaction fixed

Revisited the `ImportHandler.php`/`mail.mail_compose.composeFromDraft` item flagged-but-left-alone
in the `send()`/`merge()` split entries above, after ralf pushed back ("what is with the
ImportHandler class, it also calls mail_compose?"). Traced via `git log --follow -p` on the old
`class.mail_compose.inc.php`: `composeFromDraft()` was a real method on classic `mail_compose`,
deleted in commit `3b3986e27d` (2013-10-04) and never replaced - 13 years, not something this
rename touched. Confirmed genuinely unreachable: the only registered caller of
`importMessageFromVFS2DraftAndDisplay()` (`mail_hooks.inc.php`'s `message/rfc822` mime-handler, for
opening `.eml` files) never passes a `$mode`, always taking the `'display'` branch; no template or
JS anywhere ever passes a non-`'display'` mode.

Fixed anyway rather than leave a landmine: `mail/src/Ui/ImportHandler.php`'s `$mode != 'display'`
branch now redirects to `/mail/compose.php?from=composefromdraft&id=...&acc_id=...` - the same
client-side compose-from-draft flow `mail_ui::ajax_view()` already uses for an existing draft row
(Step 10's own established pattern) - instead of a menuaction that was never real. `php -l` clean.
